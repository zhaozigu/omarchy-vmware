...
ovm_fix_all_changed=0
ovm_fix_all_packages_before=''
ovm_fix_all_packages_after=''
...
printf 'The system will restart only if package or repair changes are made.\n\n'
...
ovm_fix_all_packages_before="$(pacman -Q)"
omarchy update
ovm_fix_all_packages_after="$(pacman -Q)"
if [[ "$ovm_fix_all_packages_before" != "$ovm_fix_all_packages_after" ]]; then
  ovm_fix_all_changed=1
  printf 'System package changes detected.\n'
fi
...
if (( ovm_fix_all_changed == 0 )); then
  printf '\n[8/8] Complete repair finished successfully. No changes were needed; restart skipped.\n'
else
